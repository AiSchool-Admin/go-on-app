...
CREATE INDEX IF NOT EXISTS idx_rides_origin ON rides USING GIST(origin_location);
...
CREATE INDEX IF NOT EXISTS idx_ratings_related ON ratings(related_type, related_id);
...
CREATE INDEX IF NOT EXISTS idx_snapshots_hour ON price_snapshots(hour_of_day);
...
-- =====================================================
-- HELPER FUNCTION: Find Nearby Drivers
-- =====================================================
...
-- Drivers (public read for verified, own write)
...
CREATE POLICY "Users can create driver profile" ON drivers
  FOR INSERT WITH CHECK (auth.uid() = user_id);

-- Vehicles
CREATE POLICY "Anyone can view verified vehicles" ON vehicles
  FOR SELECT USING (EXISTS (
    SELECT 1 FROM drivers WHERE drivers.id = vehicles.driver_id AND drivers.is_verified = TRUE
  ));
...
CREATE POLICY "Participants can update shipments" ON shipments
  FOR UPDATE USING (
    auth.uid() = sender_id OR
    auth.uid() IN (SELECT user_id FROM drivers WHERE id = shipments.driver_id)
  );
...
-- for: rides, shipments, drivers, notifications
-- =====================================================
